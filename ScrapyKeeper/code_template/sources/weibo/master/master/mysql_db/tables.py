...
class Post(Base):
    __tablename__ = "sinaweibo_{{root_project_name}}"
    __table_args__ = {'comment': '新浪微博_{{project_name_zh}}'}
...
class Comment(Base):
    __tablename__ = "sinaweibo_{{root_project_name}}_comment"
    __table_args__ = {'comment': '新浪微博评论信息_{{project_name_zh}}'}
    Id = Column(Integer, primary_key=True, autoincrement=True)
    date_created = Column(DateTime, default=datetime.datetime.now, comment='数据采集时间')
    date_modified = Column(DateTime, default=datetime.datetime.now, onupdate=datetime.datetime.now,
                           comment='数据更新时间')
